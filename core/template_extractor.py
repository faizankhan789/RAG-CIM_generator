...
- .pptx / .ppt    -> core/pptx_style_extractor.py   (python-pptx, slide/shape
                     object model — legacy binary .ppt can't be opened by
                     python-pptx and is reported as UnsupportedTemplateFileError,
                     same as legacy .doc)
...
SUPPORTED_EXTENSIONS = {"pdf", "docx", "doc", "pptx", "ppt", "html", "htm", "xml"}
...
def extract_style_profile(file_bytes: bytes, filename: str) -> tuple[dict, list[str]]:
    ...
    if ext in ("pptx", "ppt"):
        from core.pptx_style_extractor import extract_style_profile as _extract_pptx
        try:
            ...
            raise UnsupportedTemplateFileError(
                "Couldn't read this PowerPoint file — it may be a legacy .ppt file, "
                "corrupted, or password-protected. Please save it as .pptx or PDF and re-upload."
            ) from exc
    ...
    raise UnsupportedTemplateFileError(
        ...
        "Upload a PDF, Word (.docx), PowerPoint (.pptx), HTML, or XML file."
    )
