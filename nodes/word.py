...
def _rtf_to_text(path: Path) -> str:
    """Minimal RTF stripper — removes RTF control words."""
    import re
...
def _to_text(path: Path) -> str:
    ...
    if suffix == ".doc":
        # Try docx parser first (some .doc are actually docx)
        try:
            return _docx_to_text(path)
        except Exception:
            pass
...
async def _process_one(item: FileItem, listing_xml: str = "") -> ExtractedContent:
    try:
        ...
        log.info("  Word extracted: %r", item.label)
        ...
        log.error("  Word failed: %r — %s", item.label, exc)
...
async def word_node(state: CIMState) -> dict:
    ...
    log.info("Word: processing %d file(s)", len(files))
